...
我又看了下，还不止于此，有的`SO`还承担了这一类`SO`配置的数据库的职责，要访问某个类型`SO`的某个特定配置，也是通过该`SO`的静态方法来获取。
感觉`SO`在本项目中承担的职责过多，可能这样写起来比较方便、直接。

感觉本项目的访问权限管理比较松和乱，很多成员变量都是`public`，然后外部直接访问，这样很容易误修改之类的。但是又实现了少量`getter`，时不时又通过`getter`访问，命名成员变量本身就已经是`public`了，访问逻辑不统一。